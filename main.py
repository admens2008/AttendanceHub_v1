...
@app.route('/CancelSignout/<workdayid>', methods=['POST'])
@jwt_required()
def CancelSignout(workdayid):
    user = Worktransaction.query.filter_by(worktransid=workdayid).first()
    json_data = {}
    if user:
...
@app.route('/Cancelboth/<workdayid>', methods=['POST'])
@jwt_required()
def Cancelboth(workdayid):
    user = Worktransaction.query.filter_by(worktransid=workdayid).first()
    json_data = {}
    if user:
...
@app.route('/get_work/<workdayid>', methods=['POST'])
@jwt_required()
def get_work(workdayid):
    user = Workday.query.filter_by(workdayid=workdayid).first()
    json_data = {}
    if user:
        json_data['workday_date'] = user.workday_date
        json_data['start_time'] = user.start_time
        json_data['end_time'] =  user.end_time
        json_data['work_status'] = user.work_status
        json_data['status'] = 'success'
        json_data['message'] = 'success'
        return  jsonify(json_data), 200
    else:
...
@app.route('/deletework/<workdayid>', methods=['POST'])
@jwt_required()
def deletework(workdayid):
    data = request.get_json()
    user = Workday.query.filter_by(workdayid=workdayid).first()
    if not user:
         return jsonify({'message': 'The work doesn''t exist', 'status': 'error'}), 401
            # t = Test.query.filter_by(userid=user_id).all()
    user2 = Worktransaction.query.filter_by(workdayid=workdayid).all()
    for u in user2:
        db.session.delete(u)
    db.session.delete(user)
    db.session.commit()
...
def convert_to_12_hour_format(time_str):
    # Parse the 24-hour format time string to a datetime object
    time_obj = datetime.strptime(time_str, '%H:%M')
    
    # Format the datetime object to a 12-hour format time string with AM/PM
    return time_obj.strftime('%I:%M %p')

def calculate_time_difference(work_start_time_str):
    # Get the current datetime
    current_time = datetime.now()
    
    # Convert the work_start_time_str to a datetime object
    work_start_time = datetime.strptime(work_start_time_str, '%Y-%m-%d %H:%M:%S')  # Adjust format as necessary
    
    # Calculate the difference
    time_difference = work_start_time - current_time
    
    # Convert the difference to minutes
    difference_in_minutes = time_difference.total_seconds() / 60
    
    # Print the difference in minutes
    # print(f'The difference in minutes is: {difference_in_minutes:.2f} minutes')

    return difference_in_minutes

@app.route('/clockInOut/<user_id>', methods=['POST'])
@jwt_required()
def clockInOut(user_id):
    try:
        in_status = ''
        u = User.query.filter_by(userid=user_id).first()
        if not u:
        ...
        diff = calculate_time_difference(work_start_time)
        outdiff = calculate_time_difference(work_end_time)
        print(outdiff)
        if int(diff) < 0:
            in_status = 'LATE'
        if datetime.now() <  datetime.strptime(work_start_time, '%Y-%m-%d %H:%M:%S') :
            ...
            in_status = 'LATE'
        w = Workday.query.filter_by(company_id=u.company_id, workday_date=datetime.now().date()).first()
        if w:
        ...
        if Type == 'signin':
            wt = Worktransaction(Date=datetime.now().date(),
                                ...
                                time_diff_from_starttime=diff,
                                sign_in_status=in_status,
                                sign_in_reasons=reason,
                                sign_out_time=None,
                                sign_out_status=None,
                                sign_out_reasons=None,
                                is_approved=False,
                                approved_by=None,
            ...
            if w.start_time > datetime.now():
                ...
                return jsonify(response_data), 200
            w = Worktransaction.query.filter_by(company_id=u.company_id, Date=datetime.now().date(), userid=user_id).first()
            if w:
                if int(outdiff) > 0:
                ...
                for admin in admins:
                    scheduler.add_job(id=f'send_approval_mail{datetime.now()}', func=send_approval_mail, args=(user.email, staffname, w.worktransid, 'Company Name',
...
@app.route('/saveworkday/<user_id>', methods=['POST'])
@jwt_required()
def saveworkday(user_id):
    try:
        data = request.json
        wd = data.get('workday_date', '')
        ...
        u = User.query.filter_by(userid=user_id).first()
        work_date = datetime.strptime(wd, '%Y-%m-%d').date()
        w = Workday.query.filter_by(company_id=u.company_id,workday_date=work_date).first()
        if w and mod != True:
            response_data = {
                'status': 'success',
        ...
        if  start_time == '' or end_time == '':
            ...
            # Convert 12-hour format strings back to datetime.time objects
            start_time_obj = datetime.strptime(start_time_12hr, '%I:%M %p').time()
            end_time_obj = datetime.strptime(end_time_12hr, '%I:%M %p').time()
            d_start_time = datetime.combine(work_date, start_time_obj)
            d_end_time = datetime.combine(work_date, end_time_obj)
        print(d_start_time)
        print(d_end_time)
        response_data = {
                'status': 'success',
                'message': 'Work day saved successfully'
            }
        user = User.query.filter_by(userid=user_id).first()
        ...
        if user and mod != True:
            ...
            w = Workday(workday_date=work_date,
                        start_time= d_start_time, end_time=d_end_time,
 ...
 """
            ...
            u = Workday.query.filter_by(workdayid=moduserid).first()
            if u:
                u.start_time = d_start_time
                u.end_time = d_end_time
...
def attendance(user_id):
    ...
    wd = Workday.query.filter_by(company_id=u.company_id,workday_date=work_date).first()
    if not wd:
        st = None
        et = None
        status =  ''
...
@app.route('/attendancereport/<user_id>', methods=['GET', 'POST'])
@login_required
def attendancereport(user_id):
    ...
    if q_param == 'name' and request.form['name'] != '':
        ...
        q2 = request.form['name2']
        q2 = datetime.strptime(q2, '%Y-%m-%d').date()
        ...
        if request.form.get('selectuser') != 'Select user':
            user_transactions = Worktransaction.query.filter(
                ...
                Worktransaction.company_id == u.company_id,
        ...
        # user = User.query.filter(User.email != u.email).order_by(desc(User.created))
        user_transactions = Worktransaction.query.filter(Worktransaction.company_id == u.company_id,
                                                         Worktransaction.Date == datetime.now().date()
                            ).order_by(desc(Worktransaction.Date)).all()
    u = User.query.filter_by(userid=user_id).first()
    if not u:
        return jsonify({'error': 'Unauthorized User'}), 401
    dict_user = {}
    us = User.query.filter_by(company_id=u.company_id).all()
    for u in us:
...
def revokehourboard(user_id):
    ...
    if q_param == 'name' and request.form['name'] != '':
        ...
        user = Worktransaction.query.filter(
                Worktransaction.Date.between(q, q2),
                Worktransaction.company_id == u.company_id,
                Worktransaction.userid == selectuser
        ...
        if wt:
            ...
            current_date = qs
            for admin in admins:
                scheduler.add_job(id=f'send_approval_mail{datetime.now()}', func=send_approval_mail, args=(user1.email, staffname, wt.worktransid, 'Company Name',
                    'Company Address', admin.email, f'{admin.last_name} {admin.first_name}', wt.userid, current_date), trigger='date', run_date=run_time)
            db.session.commit()
        user = Worktransaction.query.filter_by(company_id=u.company_id).order_by(desc(Worktransaction.Date))
    else:
        # user = User.query.filter(User.email != u.email).order_by(desc(User.created))
        user = Worktransaction.query.filter_by(company_id=u.company_id).order_by(desc(Worktransaction.Date))
    if not user:
    ...
    for u in us:
        dict_user[u.userid] = f"{u.last_name} {u.first_name}"
    return render_template('revokehourboard.html', user=user, i=0, pages=pages,
                           companyname='', user_id=user_id, dict_user=dict_user)

   

@app.route('/workhourboard/<user_id>', methods=['GET', 'POST'])
@login_required
def workhourboard(user_id):
    count = 1
    u = User.query.filter_by(userid=user_id).first()
    if not u:
        return jsonify({'error': 'Unauthorized User'}), 401
    ...
    if q_param == 'name' and request.form['name'] != '':
        ...
        user = Workday.query.filter(
                Workday.workday_date.between(q, q2), Workday.company_id==u.company_id
            ).order_by(desc(Workday.workday_date))
    else:
        # user = User.query.filter(User.email != u.email).order_by(desc(User.created))
        user = Workday.query.filter(Workday.company_id==u.company_id
            ).order_by(desc(Workday.workday_date))
    if not user:
        return jsonify({'error': 'Unauthorized User'}), 401
    u = User.query.filter_by(userid=user_id).first()
    ...
    return render_template('workhourboard.html', user=user, i=0, pages=pages,
                           companyname='', user_id=user_id)
